...
enum NewMessageHTMLClassifier {
  ...
  static func hasEntity(_ text: String) -> Bool {
    text.range(of: self.entityPattern, options: .regularExpression) != nil
  }
  ...
  static func singleParagraphPlainText(_ raw: String) -> String? {
    ...
    guard lower.hasPrefix("<p"), lower.hasSuffix("</p>")
    else {
      return nil
    }
...
private struct NewMessageHTMLParserState {
  ...
  mutating func parse() -> NewMessageParsedPayload {
    var index = self.raw.startIndex

    while index < self.raw.endIndex {
      if self.raw[index] == "<" {
        if self.raw[index...].hasPrefix("<!--") {
        ...
        self.appendTextChunk(String(self.raw[index]))
        index = self.raw.index(after: index)
        continue
      }

      let nextTag = self.raw[index...].firstIndex(of: "<") ?? self.raw.endIndex
      let textChunk = String(raw[index ..< nextTag])
      self.appendTextChunk(textChunk)
      index = nextTag
    }

    self.trimTrailingWhitespaceAndBreaks()
  ...
  private mutating func handle(_ token: NewMessageHTMLTagToken) {
    if token.name == "script" || token.name == "style" {
      if token.isClosing {
        self.suppressTextDepth = max(0, self.suppressTextDepth - 1)
      } else {
        self.suppressTextDepth += 1
      }
      return
    }

    if token.isClosing {
      self.handleClosing(token)
      return
    }

    switch token.name {
    case "br":
      self.appendNewlineIfNeeded(force: true)

    case "p":
      self.appendNewlineIfNeeded(force: false)
    ...
    case "blockquote":
      self.appendNewlineIfNeeded(force: false)
      if !token.isSelfClosing {
        self.activeTags.append(NewMessageActiveTag(name: token.name, style: .blockquote, href: nil))
      }

    case "strong", "b":
      if !token.isSelfClosing {
        self.activeTags.append(NewMessageActiveTag(name: token.name, style: .bold, href: nil))
      }

    case "em", "i":
      if !token.isSelfClosing {
        self.activeTags.append(NewMessageActiveTag(name: token.name, style: .italic, href: nil))
      }

    case "s":
      if !token.isSelfClosing {
        self.activeTags.append(NewMessageActiveTag(name: token.name, style: .strike, href: nil))
      }

    case "code":
      if !token.isSelfClosing {
        self.activeTags.append(NewMessageActiveTag(name: token.name, style: .code, href: nil))
      }

    case "pre":
      self.appendNewlineIfNeeded(force: false)
      if !token.isSelfClosing {
        self.activeTags.append(NewMessageActiveTag(name: token.name, style: [.pre, .code], href: nil))
      }

    case "a":
      if !token.isSelfClosing {
        let href = self.sanitizeHref(token.attributes["href"])
        let style: NewMessageStyleBits = href == nil ? [] : .link
        self.activeTags.append(NewMessageActiveTag(name: token.name, style: style, href: href))
      }
    ...
    case "ul", "ol":
      ...
      if !token.isSelfClosing {
        ...
        self.listStack.append(NewMessageListContext(name: token.name, nextIndex: nextIndex))
      }

    case "li":
      self.appendNewlineIfNeeded(force: false)
      self.appendListPrefix()
      if token.isSelfClosing {
        self.appendNewlineIfNeeded(force: true)
      }
  ...
  private mutating func handleClosing(_ token: NewMessageHTMLTagToken) {
    ...
    case "strong", "b", "em", "i", "s", "code", "a":
      self.popLastActiveTag(named: token.name)
    ...
    if token.name == "ul" || token.name == "ol" {
      self.popLastList(named: token.name)
    }
  }

  private mutating func appendListPrefix() {
    if self.listStack.isEmpty {
      self.appendStyledText("• ")
      return
    }

    let lastIndex = self.listStack.count - 1
    if self.listStack[lastIndex].name == "ol" {
      let marker = "\(listStack[lastIndex].nextIndex). "
      self.listStack[lastIndex].nextIndex += 1
      self.appendStyledText(marker)
    } else {
      self.appendStyledText("• ")
    }
  }
  ...
  private mutating func appendTextChunk(_ chunk: String) {
    guard self.suppressTextDepth == 0 else { return }
    guard !chunk.isEmpty else { return }

    let decoded = NewMessageHTMLEntityDecoder.decode(chunk)
    if decoded.isEmpty { return }

    if self.isInsideTag(named: "pre") {
      self.appendStyledText(decoded)
      return
    }
    ...
    for ch in decoded {
      ...
      if pendingWhitespace {
        let previous = normalized.last ?? self.output.last
    ...
    self.appendStyledText(normalized)
  }

  private mutating func appendStyledText(_ text: String) {
    guard !text.isEmpty else { return }

    let startUTF16 = self.output.utf16.count
    self.output.append(text)
    let endUTF16 = self.output.utf16.count

    let activeBits = self.currentStyleBits()
    let activeHref = self.currentHref()
  ...
  private func parseTag(at start: String.Index) -> (NewMessageHTMLTagToken, String.Index)? {
    ...
    var content = String(raw[raw.index(after: start) ..< end]).trimmingCharacters(
    ...
    let attributes = self.parseAttributes(attributesRaw)
    ...
    return (token, self.raw.index(after: end))
  }
  ...
  private func sanitizeHref(_ href: String?) -> String? {
    ...
    let decoded = NewMessageHTMLEntityDecoder.decode(href).trimmingCharacters(
      in: .whitespacesAndNewlines
    )
