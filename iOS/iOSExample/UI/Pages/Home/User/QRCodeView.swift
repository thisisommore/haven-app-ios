    ...
    ZStack {
      ...
      VStack {
        HStack {
          ...
          Button {
            self.torchOn.toggle()
            self.toggleTorch(on: self.torchOn)
            ...
            Image(systemName: self.torchOn ? "flashlight.on.fill" : "flashlight.off.fill")
              .font(.system(size: 14, weight: .bold))
              .foregroundColor(self.torchOn ? .yellow : .white)
...
struct CameraPreviewView: UIViewControllerRepresentable {
  ...
  func makeCoordinator() -> Coordinator {
    Coordinator(onCodeFound: self.onCodeFound)
  }
...
final class CameraViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate {
  ...
  func metadataOutput(
    ...
    guard !self.hasFoundCode,
          let object = metadataObjects.first as? AVMetadataMachineReadableCodeObject,
          let code = object.stringValue
    else { return }

    self.hasFoundCode = true
