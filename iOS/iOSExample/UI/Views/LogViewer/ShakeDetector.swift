  ...
  func body(content: Content) -> some View {
    content
      .onShake {
        ...
        self.showLogViewerAlert = true
      }
      .alert("Developer Console", isPresented: self.$showLogViewerAlert) {
        Button("Open Log Viewer") {
          self.showLogViewerSheet = true
        }
        Button("Cancel", role: .cancel) {}
      } message: {
        Text("Do you want to open the log viewer?")
      }
      .sheet(isPresented: self.$showLogViewerSheet) {
        LogViewerSheet()
      }
  }
}
