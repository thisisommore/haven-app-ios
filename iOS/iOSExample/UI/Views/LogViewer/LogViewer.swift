...
class LogViewer: ObservableObject {
  ...
  init(numLines: Int = 1000) {
    self.size = numLines
    self.Messages.reserveCapacity(self.size)
...
class OpenFileCloner: ObservableObject {
  ...
  init(fd: Int32) {
    ...
    dup2(fd, self.origOut.fileHandleForWriting.fileDescriptor)
    ...
    dup2(self.input.fileHandleForWriting.fileDescriptor, fd)
    ...
    // data to read.
    self.input.fileHandleForReading.readabilityHandler = { handle in
      let data = handle.availableData
      if data.isEmpty {
        return
      }
